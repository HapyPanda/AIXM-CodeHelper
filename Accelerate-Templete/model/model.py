...
class Codebook(nn.Module):
    def __init__(self, n_codes, embedding_dim):
        super().__init__()
        self.register_buffer("embeddings", torch.randn(n_codes, embedding_dim)) # n_codes,embed
        self.register_buffer("N", torch.zeros(n_codes))     # n_codes 
        self.register_buffer("z_avg", self.embeddings.data.clone())  # n_codes,embed
    ...
    def _init_embeddings(self, z):
        ...
        flat_inputs = shift_dim(z, 1, -1).flatten(end_dim=-2) # bcthw->bthwc->(bthw)c
        y = self._tile(flat_inputs)  # 用z加上一定的噪声来初始化codebook

        d = y.shape[0]
        _k_rand = y[torch.randperm(y.shape[0])][: self.n_codes] # 随机打乱
        if dist.is_initialized():  # 将进程0的_k_rand广播到其他所有进程中同名变量
    ...
    def forward(self, z):
        ...
        encoding_indices = encoding_indices.view(z.shape[0], *z.shape[2:]) # bthw

        embeddings = F.embedding(encoding_indices, self.embeddings) # bthw embed
        embeddings = shift_dim(embeddings, -1, 1) # b embed t h w 
        ...
        if self.training:
            ...
            encode_sum = flat_inputs.t() @ encode_onehot # c (bthw) * (bthw) n_codes = c n_codes
        ...
        return dict(
            embeddings=embeddings_st, # b embed t h w 
            encodings=encoding_indices, # bthw
